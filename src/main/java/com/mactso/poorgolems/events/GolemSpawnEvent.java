	...
	public void handleGolemSpawnEvent(EntityJoinLevelEvent event) {
		...
		if (event.getLevel() instanceof ServerLevel sLevel && 
			...
			if (l.size() >= MyConfig.getIronGolemChunkLimit()) {
				...
				event.getEntity().hurt(event.getEntity().damageSources().outOfWorld() , 200);		
					Utility.debugMsg(1, event.getEntity().blockPosition(), "Poor Golems: "+ MyConfig.getIronGolemChunkLimit()+" Golem Chunk Limit Blocked Attempted Golem Spawn.");
